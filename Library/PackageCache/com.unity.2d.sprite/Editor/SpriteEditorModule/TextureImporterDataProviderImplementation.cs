...
namespace UnityEditor.U2D.Sprites
{
    ...
    internal class SpriteCustomDataTransfer : SpriteDataProviderBase, ISpriteCustomDataProvider
    {
        public SpriteCustomDataTransfer(TextureImporterDataProvider dp) : base(dp)
        {}

        public void SetData(GUID guid, string newData)
        {
            var data = ((SpriteDataExt)dataProvider.GetSpriteData(guid));
            if (data != null)
                data.customData = newData;
        }

        public string GetData(GUID guid)
        {
            return ((SpriteDataExt)dataProvider.GetSpriteData(guid))?.customData;
        }

        public static void Apply(SerializedProperty sp, string customData)
        {
